/* =========================================================
   Beta 243 — classement partagé et amis fiables
   Le serveur devient l'unique vérité dès qu'il a répondu.
   ========================================================= */
(function beta243SocialAndRankingTruth(){
  const BETA243_VERSION = "1.0.0-beta.243";
  const RESYNC_KEY = `${STORAGE_KEY}_beta243_exact_score_resync`;
  ...
  scoreForScope = function beta243ScoreForScope(scope = "daily") {
  ...
  myPlayerProfile = function beta243MyPlayerProfile() {
  ...
  scoreOfPlayer = function beta243ScoreOfPlayer(player = {}, scope = "daily") {
  ...
  leaderboardRows = function beta243LeaderboardRows(scope = state.rankScope || "daily") {
  ...
  scorePayloadForMystery = function beta243ScorePayloadForMystery(mysteryId) {
  ...
  fetchServerLeaderboard = async function beta243FetchServerLeaderboard(scope = "daily", { force = false } = {}) {
    if (!isOnline) return;
    const now = Date.now();
    const status = state.serverLeaderboardStatus?.[scope] || {};
    ...
    queueSaveState(100);
    try {
      if (typeof beta128FlushScoreOutbox === "function") await beta128FlushScoreOutbox({ force, reason: `beta243:${scope}` }).catch(() => {});
      await syncMyProfileToServer({ source: `leaderboard-${scope}-beta243` }).catch(() => null);
  ...
  fetchServerFriends = async function beta243FetchServerFriends({ force = false } = {}) {
    ...
    queueSaveState(100);
    try {
      await syncMyProfileToServer({ source: "social-state-beta243" }).catch(() => null);
      const params = new URLSearchParams({
        playerId: playerIdMe(),
        friendCode: friendCode(),
        pseudo: currentPseudo(),
  ...
  function rowsMarkup243(rows = []) {
    ...
    return rows.map(row => `<div class="hd242-rank-row${row.me ? " me" : ""}"><span class="hd242-position">${row.rank}</span><button type="button" class="hd242-player" ${row.me ? "disabled" : `data-view-profile="${escapeHtml(row.id)}"`}><strong>${escapeHtml(row.name || row.pseudo || "Joueur")}${row.me ? " · toi" : ""}</strong><small>${row.me ? "score confirmé par le serveur" : "score partagé"}</small></button><b>${Number(row.score || 0)} pt</b></div>`).join("");
  }

  renderRank = function beta243RenderRank() {
    ...
    const help = scope === "week" ? "Somme des scores obtenus depuis lundi."
      ...
      : scope === "friends" ? "Score du jour, uniquement entre amis acceptés."
    ...
    renderShell(`<header class="topbar hd242-rank-topbar"><button type="button" data-home aria-label="Retour">←</button><div><p class="eyebrow">Classement</p><h1>${escapeHtml(periodLabel)}</h1></div><button type="button" class="hd242-profile-button" data-open-profile>${escapeHtml((state.pseudo || "P").charAt(0).toUpperCase())}</button></header>
      ...
      <section class="card hd242-leaderboard"><div class="section-title-row"><div><span class="card-label">${scope === "friends" ? "Entre amis" : "Classement général"}</span><h2>${rows.length} joueur${rows.length > 1 ? "s" : ""}</h2></div></div><div class="hd242-rank-list">${rowsMarkup243(rows)}</div></section>
  ...
  function resyncExactScoresOnce() {
    ...
    if (already === BETA243_VERSION) return;
    try {
      Object.keys(state.solvedMysteries || {}).forEach(mysteryId => {
        if (typeof beta128QueueScorePayload === "function") beta128QueueScorePayload(scorePayloadForMystery(mysteryId), "beta243-exact-score-repair");
      });
      localStorage.setItem(RESYNC_KEY, BETA243_VERSION);
      if (typeof beta128FlushScoreOutbox === "function") beta128FlushScoreOutbox({ force: true, reason: "beta243-repair" }).catch(() => {});
    } catch {}
  }
  ...
  try {
    ...
    state.serverLeaderboardStatus = {};
    state.rankSemanticsVersion = BETA243_VERSION;
    state.beta243SocialTruth = true;
    queueSaveState(100);
    ...
    window.HistoDaily = { ...(window.HistoDaily || {}), version: BETA243_VERSION, serverScoreAuthority: true, canonicalFriends: true };
  } catch (error) {
    try { console.warn("beta243 social truth", error); } catch {}
  }
})();
