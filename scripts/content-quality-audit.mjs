...
for(const [id,pack] of Object.entries(catalogue.packs)){
  ...
  quiz.forEach((q,index)=>{
    ...
    const whyFloor=d==='english'?8:12;
    if(wordCount(why)<whyFloor) issues.push({severity:'error',code:'quiz-explanation-short',lessonId:id,discipline:d,questionIndex:index,value:wordCount(why),target:whyFloor});
...
for(const mystery of catalogue.mysteries){
  ...
  const promptFloor=mystery.discipline==='english'?10:20;
  if(wordCount(mystery.prompt)<promptFloor) issues.push({severity:'error',code:'mystery-prompt-short',mysteryId:mystery.id,discipline:mystery.discipline,value:wordCount(mystery.prompt),target:promptFloor});
