...
async function handleLeaderboard(req, res) {
  ...
  if (!configured()) return send(res, 200, { ok: true, scope, periodKey, mode: 'local-preview', rows: [], note: 'Classement en ligne non connecté pour l’instant : seuls les scores locaux s’affichent.' });
  try {
    ...
    return send(res, 200, { ok: true, scope, periodKey, mode: 'supabase', rows: aggregate(enriched, scope), profileSync: 'enabled' });
  } catch (error) {
    return send(res, 200, { ok: true, scope, periodKey, mode: 'supabase-error', rows: [], note: 'Classement en ligne indisponible pour le moment.' });
  }
}
...
async function handleFriends(req, res) {
  ...
  if (!configured()) return send(res, 200, { ok: true, mode: 'local-preview', friends: [], features: { addByCode: true, removeByCode: true, profiles: true, leaderboards: true, chat: false, inviteLinks: true }, message: 'Amis conservés sur cet appareil. La synchronisation en ligne s’activera quand le service sera branché.' });
  try {
    if (method === 'POST') {
      ...
      const canonicalCode = cleanFriendCode(profile?.friend_code || inputCode);
      const row = {
        player_id: playerId,
      ...
      if (friendCodeMatches(row.friend_code, myCode)) return send(res, 400, { ok: false, message: 'Impossible de s’ajouter soi-même.' });
      ...
      if (row.friend_player_id) {
        await request(`hd_friends?player_id=eq.${encodeURIComponent(playerId)}&friend_player_id=eq.${encodeURIComponent(row.friend_player_id)}`, { method: 'DELETE', prefer: 'return=minimal' }).catch(() => null);
      }
      const suffix = codeSuffix(row.friend_code);
      if (suffix) {
        await request(`hd_friends?player_id=eq.${encodeURIComponent(playerId)}&friend_code=ilike.${encodeURIComponent(`*-${suffix}`)}`, { method: 'DELETE', prefer: 'return=minimal' }).catch(() => null);
      }
      await request('hd_friends?on_conflict=player_id,friend_code', { method: 'POST', prefer: 'resolution=merge-duplicates,return=representation', body: [row] });
    ...
    return send(res, 200, { ok: true, mode: 'supabase', stored: true, friends: enriched, features: { addByCode: true, removeByCode: true, profiles: true, leaderboards: true, chat: false, inviteLinks: true, profileRefresh: true, selfProfileUpsert: true } });
  } catch (error) {
...
function handleHealth(req, res) {
  return send(res, 200, {
    ...
    features: ['daily-mystery', 'course-flow', 'leaderboards', 'friends', 'local-backup', 'pwa']
  });
}
