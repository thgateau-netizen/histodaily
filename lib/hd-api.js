...
async function beta249UpsertBestScore(payload = {}) {
  ...
  // Repli beta250 sans dépendance à une contrainte UNIQUE composite. Certaines
  // installations historiques de Supabase n'ont pas encore cette contrainte :
  // on lit d'abord, puis on insère simplement ou on fusionne les lignes.
  ...
  const existing = await request(`${base}&select=*&order=score.desc&limit=20`).catch(() => []);
  if (Array.isArray(existing) && existing.length) {
    const best = existing.reduce((winner, row) => Number(row.score || 0) > Number(winner?.score || 0) ? row : winner, existing[0]);
    const merged = {
      ...payload,
      score: Math.max(Number(payload.score || 0), Number(best?.score || 0)),
      hints: Math.min(Number(payload.hints || 0), Number.isFinite(Number(best?.hints)) ? Number(best.hints) : Number(payload.hints || 0)),
      tries: Math.min(Math.max(1, Number(payload.tries || 1)), Math.max(1, Number(best?.tries || payload.tries || 1))),
      level: Math.max(Number(payload.level || 1), Number(best?.level || 1)),
      xp: Math.max(Number(payload.xp || 0), Number(best?.xp || 0)),
      solved_count: Math.max(Number(payload.solved_count || 0), Number(best?.solved_count || 0)),
      streak: Math.max(Number(payload.streak || 0), Number(best?.streak || 0)),
      solved_at: Number.isFinite(Date.parse(best?.solved_at || '')) && Date.parse(best.solved_at) < Date.parse(payload.solved_at)
        ? best.solved_at : payload.solved_at
    };
    const updated = await request(`${base}&select=*`, {
      method: 'PATCH', prefer: 'return=representation', body: merged
    }).catch(() => []);
    if (updated?.[0]) return { rows: updated, mode: 'supabase-compatible-merge' };
  } else {
    const inserted = await request('hd_scores?select=*', {
      method: 'POST', prefer: 'return=representation', body: [payload]
    }).catch(() => []);
    if (inserted?.[0]) return { rows: inserted, mode: 'supabase-compatible-insert' };
  }

  // Dernier recours pour les schémas déjà munis de la contrainte composite.
  ...
  const current = await request(`${base}&select=*&order=score.desc&limit=1`).catch(() => []);
  if (current?.[0]) return { rows: current, mode: 'supabase-compatible-readback' };
...
/* =========================================================
   Beta 250 — réparation ponctuelle des relations locales héritées
   ========================================================= */
const beta250BaseHandleFriends = handleFriends;
handleFriends = async function beta250HandleFriends(req, res) {
  const method = String(req.method || 'GET').toUpperCase();
  const body = readBody(req);
  const action = safeText(body.action || body.intent || '', 40);
  if (method !== 'POST' || action !== 'repair-legacy-relation' || !configured()) {
    return beta250BaseHandleFriends(req, res);
  }
  try {
    const own = await beta249MergeProfile({
      ...body,
      friendCode: body.myFriendCode || body.friendCodeSelf || body.selfFriendCode || ''
    });
    const me = await beta244ResolveIdentity(
      own.canonicalPlayerId || body.playerId || '',
      own.canonicalFriendCode || body.myFriendCode || '',
      body.pseudo || 'Joueur'
    );
    const requestedId = safeText(body.friendPlayerId || body.targetPlayerId || '', 90);
    const requestedCode = cleanFriendCode(body.friendCode || body.targetFriendCode || '');
    let targetProfile = requestedId ? await profileByPlayerId(requestedId).catch(() => null) : null;
    if (!targetProfile && requestedCode) targetProfile = await profileByFriendCode(requestedCode).catch(() => null);
    if (!targetProfile) return send(res, 404, { ok: false, stored: false, message: 'Profil ami introuvable : relation locale conservée.' });
    const other = {
      playerId: safeText(targetProfile.player_id || '', 90),
      friendCode: cleanFriendCode(targetProfile.friend_code || requestedCode),
      pseudo: safeText(targetProfile.pseudo || body.friendPseudo || 'Joueur', 32),
      profile: targetProfile
    };
    if (!me.playerId || !me.friendCode || !other.playerId || !other.friendCode) {
      return send(res, 400, { ok: false, stored: false, message: 'Identités incomplètes pour réparer la relation.' });
    }
    if (me.playerId === other.playerId || friendCodeMatches(me.friendCode, other.friendCode)) {
      return send(res, 400, { ok: false, stored: false, message: 'Relation vers soi-même refusée.' });
    }
    await upsertFriendPair(me, other);
    const friends = await currentFriendRows(me.playerId).catch(() => []);
    const requests = splitRequestsForClient(await friendRequestRowsForPlayer(me.playerId, me.friendCode).catch(() => []));
    return send(res, 200, {
      ok: true, stored: true, mode: 'supabase', authoritative: true,
      canonicalPlayerId: me.playerId, canonicalFriendCode: me.friendCode,
      friends, requests, legacyRelationRepaired: true,
      message: `${other.pseudo} a été restauré dans les amis en ligne.`
    });
  } catch {
    return send(res, 200, { ok: true, stored: false, mode: 'supabase-error', authoritative: false, friends: [], message: 'Réparation de la relation différée.' });
  }
};

const beta250BaseFeatureFlags = requestFeatureFlags;
requestFeatureFlags = function beta250RequestFeatureFlags() {
  return { ...beta250BaseFeatureFlags(), compatibleScoreInsert: true, legacyFriendRepair: true, aggregateModeCompatibility: true };
};


/* =========================================================
   Beta 251 — repli de schéma réellement tolérant
   - un échec de fusion du profil ne bloque plus l'envoi du score
   - le classement peut lire les anciens schémas par created_at
   - les amis sont toujours résolus depuis l'identité canonique
   ========================================================= */
function beta251CaptureResponse() {
  let captured = null;
  return {
    response: {
      setHeader() {},
      status(code) { return { json(payload) { captured = { code, payload }; return captured; } }; }
    },
    get value() { return captured; }
  };
}

function beta251RangeFor(context = {}, req = {}) {
  const suppliedStart = safeIsoBoundary(req.query?.rangeStart || '');
  const suppliedEnd = safeIsoBoundary(req.query?.rangeEnd || '');
  const now = new Date();
  const tomorrow = new Date(now.getFullYear(), now.getMonth(), now.getDate() + 1).toISOString();
  if (suppliedStart && suppliedEnd) return { start: suppliedStart, end: suppliedEnd };
  if (context.periodScope === 'week') return { start: startOfWeekISO(now), end: suppliedEnd || tomorrow };
  if (context.periodScope === 'year') return { start: startOfYear(now), end: suppliedEnd || tomorrow };
  return { start: suppliedStart || new Date(now.getFullYear(), now.getMonth(), now.getDate()).toISOString(), end: suppliedEnd || tomorrow };
}

async function beta251ReadScoreRows(context = {}, req = {}) {
  const periodKey = safeText(req.query?.periodKey || todayKey(), 20);
  const range = beta251RangeFor(context, req);
  const limit = context.periodScope === 'year' ? 2500 : context.periodScope === 'week' ? 1200 : 500;
  const modernFilter = context.periodScope === 'daily'
    ? `&period_key=eq.${encodeURIComponent(periodKey)}`
    : `&solved_at=gte.${encodeURIComponent(range.start)}&solved_at=lt.${encodeURIComponent(range.end)}`;
  const legacyFilter = `&created_at=gte.${encodeURIComponent(range.start)}&created_at=lt.${encodeURIComponent(range.end)}`;
  const candidates = [
    `hd_scores?select=player_id,pseudo,friend_code,mystery_id,period_key,difficulty,score,level,xp,solved_count,streak,hints,tries,solved_at,created_at${modernFilter}&order=score.desc&limit=${limit}`,
    `hd_scores?select=player_id,pseudo,friend_code,mystery_id,period_key,difficulty,score,solved_at,created_at${modernFilter}&order=score.desc&limit=${limit}`,
    `hd_scores?select=player_id,pseudo,score,created_at${legacyFilter}&order=score.desc&limit=${limit}`
  ];
  let lastError = null;
  for (const path of candidates) {
    try {
      const rows = await request(path);
      if (Array.isArray(rows)) return { rows: normalizeScoreRows(rows), compatibilityPath: path.includes('friend_code') ? 'modern' : 'legacy' };
    } catch (error) { lastError = error; }
  }
  throw lastError || new Error('No readable score schema');
}

async function beta251BuildLeaderboard(req = {}) {
  const context = beta245LeaderboardContext(req);
  const read = await beta251ReadScoreRows(context, req);
  let rows = read.rows;
  let canonicalPlayerId = '';
  let canonicalFriendCode = '';
  let friendCount = null;
  ...
  if (context.audience === 'friends') {
    const identity = await beta244ResolveIdentity(
      safeText(req.query?.playerId || '', 90),
      cleanFriendCode(req.query?.myFriendCode || req.query?.friendCode || ''),
      safeText(req.query?.pseudo || 'Joueur', 32)
    );
    canonicalPlayerId = identity.playerId;
    canonicalFriendCode = identity.friendCode;
    const friends = canonicalPlayerId ? await currentFriendRows(canonicalPlayerId).catch(() => []) : [];
    friendCount = friends.length;
    participants = beta244FriendParticipantRows(identity, friends);
    const ids = new Set(participants.map(row => safeText(row.player_id || '', 90)).filter(Boolean));
    const codes = new Set(participants.map(row => cleanFriendCode(row.friend_code || '')).filter(Boolean));
    rows = rows.filter(row => ids.has(safeText(row.player_id || '', 90)) || codes.has(cleanFriendCode(row.friend_code || '')));
  }

  const profiles = await profileMapForRows(rows).catch(() => new Map());
  const enriched = enrichScoreRowsWithProfiles(rows, profiles);
  const aggregated = aggregate(enriched, context.periodScope);
  const responseRows = context.audience === 'friends' ? beta244MergeFriendLeaderboard(aggregated, participants) : aggregated;
  return {
    ok: true,
    scope: context.rawScope,
    periodScope: context.periodScope,
    audience: context.audience,
    periodKey: safeText(req.query?.periodKey || todayKey(), 20),
    mode: read.compatibilityPath === 'legacy' ? 'supabase-legacy-compatible' : 'supabase-compatible',
    authoritative: true,
    compatibilityPath: read.compatibilityPath,
    canonicalPlayerId,
    canonicalFriendCode,
    friendCount,
    rows: responseRows,
    generatedAt: new Date().toISOString(),
    note: 'Classement partagé lu depuis Supabase.'
  };
}

const beta251BaseHandleLeaderboard = handleLeaderboard;
handleLeaderboard = async function beta251HandleLeaderboard(req, res) {
  if (!configured()) return beta251BaseHandleLeaderboard(req, res);
  const capture = beta251CaptureResponse();
  try { await beta251BaseHandleLeaderboard(req, capture.response); } catch {}
  const base = capture.value;
  const payload = base?.payload || {};
  if (base && payload.authoritative === true && String(payload.mode || '').startsWith('supabase')) {
    return send(res, base.code || 200, payload);
  }
  try {
    return send(res, 200, await beta251BuildLeaderboard(req));
  } catch {
    if (base) return send(res, base.code || 200, payload);
    return send(res, 200, { ok: true, mode: 'supabase-error', authoritative: false, rows: [], note: 'Classement en ligne indisponible : dernier état connu conservé.' });
  }
};

function beta251ScorePayload(body = {}, identity = {}) {
  const difficulty = safeText(body.difficulty || 'moyen', 20);
  const playerId = safeText(identity.playerId || body.playerId || body.friendCode || 'local-player', 90);
  const friendCode = cleanFriendCode(identity.friendCode || body.friendCode || body.myFriendCode || '');
  return {
    player_id: playerId,
    pseudo: safeText(identity.pseudo || body.pseudo || 'Invité', 32),
    friend_code: friendCode,
    mystery_id: safeText(body.mysteryId || '', 80),
    period_key: safeText(body.dayKey || body.periodKey || todayKey(), 20),
    scope: 'daily',
    score: Math.max(0, Math.min(scoreCapForDifficulty(difficulty), Number(body.score || 0))),
    hints: Math.max(0, Number(body.hints || 0)),
    tries: Math.max(1, Number(body.tries || body.answerTries || 1)),
    difficulty,
    level: Math.max(1, Number(body.level || 1)),
    xp: Math.max(0, Number(body.xp || 0)),
    solved_count: Math.max(0, Number(body.solvedCount || 0)),
...
const beta251BaseHandleSubmit = handleSubmit;
handleSubmit = async function beta251HandleSubmit(req, res) {
  const method = String(req.method || 'POST').toUpperCase();
  if (method !== 'POST' || !configured()) return beta251BaseHandleSubmit(req, res);
  const capture = beta251CaptureResponse();
  try { await beta251BaseHandleSubmit(req, capture.response); } catch {}
  const base = capture.value;
  if (base?.payload?.stored === true && String(base.payload.mode || '').startsWith('supabase')) {
    return send(res, base.code || 200, base.payload);
  }

  const body = readBody(req);
  if (!safeText(body.mysteryId || '', 80)) return send(res, 400, { ok: false, stored: false, message: 'mysteryId requis.' });
  try {
    const identity = await beta244ResolveIdentity(
      safeText(body.playerId || '', 90),
      cleanFriendCode(body.friendCode || body.myFriendCode || ''),
      safeText(body.pseudo || 'Invité', 32)
    ).catch(() => ({
      playerId: safeText(body.playerId || body.friendCode || 'local-player', 90),
      friendCode: cleanFriendCode(body.friendCode || body.myFriendCode || ''),
      pseudo: safeText(body.pseudo || 'Invité', 32)
    }));
    const payload = beta251ScorePayload(body, identity);
    let stored = null;
    try { stored = await beta249UpsertBestScore(payload); } catch {}
    if (stored?.rows?.[0]) {
      return send(res, 200, {
        ok: true, stored: true, mode: stored.mode || 'supabase-compatible', authoritative: true,
        rows: stored.rows, canonicalPlayerId: payload.player_id, canonicalFriendCode: payload.friend_code,
        exactScore: Number(stored.rows[0].score ?? payload.score), bestScoreWins: true,
        profileWriteDeferred: true, message: 'Score enregistré ; profil resynchronisé séparément.'
      });
    }

    // Ultime compatibilité : schéma historique player_id/pseudo/score/created_at.
    const legacyRows = await request('hd_scores?select=*', {
      method: 'POST', prefer: 'return=representation', body: [{
        player_id: payload.player_id,
        pseudo: payload.pseudo,
        score: payload.score,
        created_at: payload.solved_at
      }]
    });
    if (legacyRows?.[0]) {
      return send(res, 200, {
        ok: true, stored: true, mode: 'supabase-legacy-compatible', authoritative: true,
        rows: legacyRows, canonicalPlayerId: payload.player_id, canonicalFriendCode: payload.friend_code,
        exactScore: payload.score, profileWriteDeferred: true,
        message: 'Score enregistré avec le schéma Supabase historique.'
      });
    }
  } catch {}
  if (base) return send(res, base.code || 200, base.payload);
  return send(res, 200, { ok: true, stored: false, mode: 'supabase-error', authoritative: false, message: 'Connexion indisponible : score conservé localement.' });
};

const beta251BaseFeatureFlags = requestFeatureFlags;
requestFeatureFlags = function beta251RequestFeatureFlags() {
  return {
    ...beta251BaseFeatureFlags(),
    resilientScoreWithoutProfile: true,
    legacyLeaderboardRead: true,
    canonicalFriendRead251: true,
    mobileLayoutRootFix: true
  };
};
