...
async function handleLeaderboard(req, res) {
  ...
  try {
    ...
    try {
      ...
      return send(res, 200, { ok: true, scope, periodKey, mode: 'supabase-legacy', rows: aggregate(normalizeScoreRows(fallbackRows), scope), note: 'Classement affiché en mode compatibilité. Lance la migration hd_scores beta132 dans Supabase.' });
    } catch (fallbackError) {
...
async function handleSubmit(req, res) {
  ...
  try {
    ...
    try {
      ...
      return send(res, 200, { ok: true, stored: true, mode: 'supabase-legacy', rows, message: 'Score enregistré en mode compatibilité. Lance la migration hd_scores beta132 pour tout fiabiliser.' });
    } catch {
...
async function handleFriendRequests(req, res) {
  ...
  try {
    if (method === 'POST') {
      ...
      if (deleteClauses.length) await request(`hd_friend_requests?requester_player_id=eq.${encodeURIComponent(playerId)}&status=eq.pending&or=(${deleteClauses.join(',')})`, { method: 'DELETE', prefer: 'return=minimal' }).catch(() => null);
      const payload = {
        requester_player_id: playerId,
        requester_friend_code: myFriendCode,
        requester_pseudo: requesterPseudo,
...
async function handleHealth(req, res) {
  ...
  return send(res, 200, {
    ...
    features: ['daily-mystery', 'course-flow', 'leaderboards', 'friends', 'friend-requests', 'public-profiles', 'profile-lookup', 'request-badges', 'local-backup', 'pwa', 'offline-score-outbox', 'identity-mirror', 'leaderboard-safe-mode']
  });
}
