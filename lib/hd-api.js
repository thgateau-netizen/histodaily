...
async function handleLeaderboard(req, res) {
  ...
  try {
    ...
    return send(res, 200, { ok: true, scope, periodKey, mode: 'supabase', rows: aggregate(enriched, scope), profileSync: 'beta64' });
  } catch (error) {
...
function handleHealth(req, res) {
  return send(res, 200, {
    ...
    version: VERSION,
    target: 'vercel',
    deployment: { apiMode: 'file-router-under-hobby-limit', hobbySafe: true, serverlessFunctions: 10, routeFix: 'beta64 learning quiz fix plus beta63 social cleanup' },
    content: {
