const { mysteries, dailyMystery, dailyMysteryForKey, dayIndexFromKey } = require('./hd-data');
...
function scoreFloorForDifficulty(value = '') {
  const difficulty = String(value || '').toLowerCase();
  if (difficulty === 'facile') return 35;
  if (difficulty === 'difficile') return 55;
  if (difficulty === 'expert') return 70;
  return 45;
}
function canonicalSubmittedScore(difficulty = 'moyen', hints = 0, tries = 1) {
  const base = scoreCapForDifficulty(difficulty);
  const floor = scoreFloorForDifficulty(difficulty);
  return Math.max(floor, base - Math.max(0, Math.min(3, Number(hints || 0))) * 20 - Math.max(0, Math.min(49, Number(tries || 1) - 1)) * 10);
}
function recentPeriodKey(value = '', maxAgeDays = 31) {
  const key = safeText(value || todayKey(), 20);
  const index = dayIndexFromKey(key);
  const todayIndex = dayIndexFromKey(todayKey());
  if (index === null || todayIndex === null) return '';
  const age = todayIndex - index;
  return age >= 0 && age <= maxAgeDays ? key : '';
}
...
async function allLeaderboardProfiles(maxRows = 2500) {
  if (!configured()) return [];
  const pageSize = 500;
  const out = [];
  for (let offset = 0; offset < maxRows; offset += pageSize) {
    const rows = await request(`hd_profiles?select=player_id,pseudo,friend_code,level,xp,solved_count,streak,updated_at&order=updated_at.desc.nullslast&limit=${pageSize}&offset=${offset}`).catch(() => []);
...
function mergeLeaderboardProfiles(scoreRows = [], profiles = []) {
  const map = new Map();
  const keyFor = row => cleanFriendCode(row.friendCode || row.friend_code || '') || safeText(row.player_id || row.playerId || row.id || '', 90) || `name:${safeText(row.pseudo || row.name || 'Joueur', 32).toLowerCase()}`;
  for (const raw of scoreRows || []) {
    const key = keyFor(raw);
    if (!key) continue;
    map.set(key, { ...raw, score: Math.max(0, Number(raw.score || 0)) });
  }
  for (const profile of profiles || []) {
    ...
    map.set(key, {
      ...existing,
      id: existing.id || profile.player_id || profile.friend_code || key,
      player_id: existing.player_id || profile.player_id || '',
      friend_code: cleanFriendCode(existing.friend_code || profile.friend_code || ''),
      friendCode: cleanFriendCode(existing.friendCode || existing.friend_code || profile.friend_code || ''),
      pseudo: safeText(profile.pseudo || existing.pseudo || 'Joueur', 32),
      score: Math.max(0, Number(existing.score || 0)),
      level: Math.max(1, Number(profile.level || existing.level || 1)),
      xp: Math.max(0, Number(profile.xp || existing.xp || 0)),
      solved: Math.max(0, Number(profile.solved_count || existing.solved || 0)),
      solved_count: Math.max(0, Number(profile.solved_count || existing.solved_count || 0)),
      streak: Math.max(0, Number(profile.streak || existing.streak || 0))
    });
  }
  return Array.from(map.values())
    .sort((a, b) => Number(b.score || 0) - Number(a.score || 0) || String(a.pseudo || '').localeCompare(String(b.pseudo || ''), 'fr'))
...
async function handleLeaderboard(req, res) {
  ...
  try {
    ...
    let ranked = aggregate(enriched, scope);

    // RC29 : le classement doit représenter les joueurs inscrits, pas seulement
    // ceux qui ont déjà marqué des points sur la période.
    const registered = await allLeaderboardProfiles();
    if (scope === 'friends') {
      const playerId = safeText(req.query?.playerId || '', 90);
      const friendCodes = String(req.query?.friendCodes || '').split(',').map(cleanFriendCode).filter(Boolean);
      const friendIds = String(req.query?.friendIds || '').split(',').map(v => safeText(v, 90)).filter(Boolean);
      ...
      const suffixSet = new Set(friendCodes.map(codeSuffix).filter(Boolean));
      const idSet = new Set(friendIds);
      const visibleProfiles = registered.filter(profile => profile.player_id === playerId || idSet.has(profile.player_id) || codeSet.has(cleanFriendCode(profile.friend_code)) || suffixSet.has(codeSuffix(profile.friend_code)));
      ranked = mergeLeaderboardProfiles(ranked, visibleProfiles);
    } else {
      ranked = mergeLeaderboardProfiles(ranked, registered);
    }
    return send(res, 200, { ok: true, scope, periodKey, mode, rows: ranked, profileSync: 'enabled', registeredPlayers: ranked.length });
  } catch (error) {
    try {
      const fallbackPath = legacyQueryFor(scope, periodKey);
      const fallbackRows = fallbackPath ? await request(fallbackPath) : [];
      return send(res, 200, { ok: true, scope, periodKey, mode: 'supabase-legacy', rows: aggregate(normalizeScoreRows(fallbackRows), scope), note: 'Classement disponible en mode de compatibilité.' });
    } catch (fallbackError) {
...
async function handleSubmit(req, res) {
  ...
  if (body.rankingEligible === false || body.daily === false || body.archive === true) return send(res, 200, { ok: true, stored: false, skipped: true, terminal: true, mode: 'not-ranked', message: 'Archive conservée hors classement.' });
  const periodKey = recentPeriodKey(body.dayKey || body.periodKey || todayKey());
  if (!periodKey) return send(res, 400, { ok: false, stored: false, terminal: true, message: 'Période de score invalide ou trop ancienne.' });
  const mysteryId = safeText(body.mysteryId || '', 80);
  // RC29 : la rotation quotidienne est personnalisée côté client pour ne jamais
  // recycler un dossier déjà réussi tant qu'il reste de l'inédit. Le serveur
  // valide donc que le dossier existe et que la date est une période quotidienne valide,
  // plutôt que d'imposer un unique mystère global à tous les joueurs.
  const expectedMystery = mysteries.find(item => item.id === mysteryId) || null;
  if (!expectedMystery) return send(res, 200, { ok: true, stored: false, skipped: true, terminal: true, mode: 'not-ranked', message: 'Dossier quotidien inconnu.' });
  const incomingDifficulty = safeText(expectedMystery.difficulty || 'moyen', 20);
  const scoreCap = scoreCapForDifficulty(incomingDifficulty);
  const score = canonicalSubmittedScore(incomingDifficulty, body.hints, body.tries || body.answerTries);
  ...
  const payload = {
    ...
    scope: 'daily',
    score,
    hints: Math.max(0, Math.min(3, Number(body.hints || 0))),
    tries: Math.max(1, Math.min(50, Number(body.tries || body.answerTries || 1))),
    ...
    solved_at: `${periodKey}T12:00:00.000Z`
  };
  ...
  try {
    ...
    try {
      ...
      return send(res, 200, { ok: true, stored: true, mode: 'supabase-legacy', rows, canonicalPlayerId: playerId, canonicalFriendCode: friendCode, message: 'Score enregistré en mode de compatibilité.' });
    } catch {
...
async function handleMe(req, res) {
  ...
  return send(res, 200, { ok: true, version: VERSION, mode: configured() ? 'server-ready' : 'local-profile', profile: { pseudo: 'Invité', level: 1, xp: 0, gems: 0, streak: 0, features: ['friends','leaderboards','public_profiles','invite_links','no_chat','profile_save'] } });
}
async function handleReset(req, res) {
  ...
  // Sans authentification de compte, une route de suppression prenant seulement
  // un playerId permettrait à un tiers d'effacer le score d'un autre joueur.
  // La progression se réinitialise localement dans l'application ; les nettoyages
  // serveur restent réservés au script administrateur pré-V1 avec ROLLBACK.
  return send(res, 403, {
    ok: false,
    stored: false,
    terminal: true,
    mode: 'admin-only',
    message: 'La suppression des scores en ligne est réservée à l’administration.'
  });
}
...
handleHealth = async function beta145HandleHealth(req, res) {
  ...
  try {
    ...
    return send(res, 200, {
      ...
      content: { readyCourses: 219, publicMysteries: mysteries.length, dailyRewardGems: 1, archiveDaysVisible: 7 },
...
handleSubmit = async function beta249HandleSubmit(req, res) {
  ...
  const body = readBody(req);
  if (body.rankingEligible === false || body.daily === false || body.archive === true) {
    return send(res, 200, {
      ok: true, stored: false, skipped: true, terminal: true, mode: 'not-ranked',
      message: 'Archive conservée hors classement.'
    });
  }
  ...
  if (!periodKey) {
    return send(res, 400, {
      ok: false, stored: false, terminal: true,
      message: 'Période de score invalide ou trop ancienne.'
    });
  }

  const mysteryId = safeText(body.mysteryId || '', 80);
  if (!mysteryId) return send(res, 400, { ok: false, stored: false, terminal: true, message: 'mysteryId requis.' });
  // RC46 : la rotation quotidienne est personnalisée dans l'app. Le backend
  // valide l'appartenance au catalogue canonique plutôt qu'un ID global par date.
  const expectedMystery = mysteries.find(item => item.id === mysteryId) || null;
  if (!expectedMystery) {
    return send(res, 200, {
      ok: true, stored: false, skipped: true, terminal: true, mode: 'not-ranked',
      message: 'Dossier quotidien inconnu du catalogue.'
    });
  }

  const difficulty = safeText(expectedMystery.difficulty || 'moyen', 20);
  const hints = Math.max(0, Math.min(3, Number(body.hints || 0)));
  const tries = Math.max(1, Math.min(50, Number(body.tries || body.answerTries || 1)));
  const exactScore = canonicalSubmittedScore(difficulty, hints, tries);
  if (!configured()) {
    return send(res, 200, {
      ok: true, stored: false, mode: 'local-preview', authoritative: false,
      score: exactScore, exactScore, periodKey, mysteryId, difficulty,
      message: 'Score conservé localement.'
    });
  }
  ...
  try {
    ...
    const payload = {
      ...
      tries,
      difficulty,
      level: Math.max(1, Number(p.level || body.level || 1)),
      xp: Math.max(0, Number(p.xp || body.xp || 0)),
      solved_count: Math.max(0, Number(p.solved_count || body.solvedCount || 0)),
      streak: Math.max(0, Number(p.streak || body.streak || 0)),
      solved_at: `${periodKey}T12:00:00.000Z`
    };
    const stored = await beta249UpsertBestScore(payload);
    if (payload.friend_code) {
      await request(`hd_scores?friend_code=eq.${encodeURIComponent(payload.friend_code)}&mystery_id=eq.${encodeURIComponent(payload.mystery_id)}&period_key=eq.${encodeURIComponent(payload.period_key)}&scope=eq.daily&player_id=neq.${encodeURIComponent(payload.player_id)}`, {
        method: 'DELETE', prefer: 'return=minimal'
      }).catch(() => null);
    }
    const storedScore = Number(stored.rows?.[0]?.score ?? payload.score);
    return send(res, 200, {
      ok: true, stored: true, mode: stored.mode, authoritative: true, rows: stored.rows,
      canonicalPlayerId: payload.player_id, canonicalFriendCode: payload.friend_code,
      score: storedScore, exactScore: storedScore, bestScoreWins: true,
      periodKey, mysteryId, difficulty,
      message: 'Meilleur score enregistré dans le classement.'
    });
  } catch {
    return send(res, 200, {
      ok: true, stored: false, mode: 'supabase-error', authoritative: false,
      score: exactScore, exactScore, periodKey, mysteryId, difficulty,
      message: 'Connexion indisponible : score conservé localement.'
    });
  }
};
...
/* HistoDaily RC12 — surface API publique minimale.
   Les outils de diagnostic et d'audit restent internes au code mais ne sont
   plus exposés sur Internet. */
const rc12BaseHandleRoute = handleRoute;
const rc12PrivateRoutes = new Set([
  'social/audit',
  'system/preflight',
  'system/selftest',
  'system/diagnostics',
...
handleRoute = async function rc12PublicHandleRoute(req, res, route) {
  if (route === 'index') return send(res, 200, { ok: true, api: 'HistoDaily', version: VERSION });
  if (rc12PrivateRoutes.has(route)) return send(res, 404, { ok: false, message: 'Route API inconnue', version: VERSION });
  return rc12BaseHandleRoute(req, res, route);
};


/* HistoDaily RC15 — Web Push */
const { handlePushRoute } = require('./hd-push');
const rc15BaseHandleRoute = handleRoute;
handleRoute = async function rc15PushHandleRoute(req, res, route) {
  if (route.startsWith('push/')) {
    const handled = await handlePushRoute(req, res, route, send);
    if (handled !== null && handled !== undefined) return handled;
  }
  return rc15BaseHandleRoute(req, res, route);
};


/* HistoDaily RC54 — centralized retention analytics.
   Event ingestion is public but strictly allow-listed and bounded. Cohort reporting is
   protected by ANALYTICS_ADMIN_TOKEN and returns aggregates only. */
const RC54_ANALYTICS_EVENTS = new Set([
  'app_open', 'push_open', 'mystery_start', 'mystery_solved', 'teaser_seen',
  'deep_dive', 'comeback_message_seen', 'onboarding_completed'
]);
function rc54SafeDay(value = '', maxAgeDays = 45) {
  const text = safeText(value || '', 10);
  if (!/^\d{4}-\d{2}-\d{2}$/.test(text)) return '';
  const parsed = dayIndexFromKey(text);
  const today = dayIndexFromKey(todayKey());
  if (parsed === null || today === null) return '';
  const age = today - parsed;
  return age >= -1 && age <= Math.max(1, Number(maxAgeDays || 45)) ? text : '';
}
function rc54SafeIso(value = '', day = '') {
  const parsed = Date.parse(String(value || ''));
  if (!Number.isFinite(parsed)) return day ? `${day}T12:00:00.000Z` : new Date().toISOString();
  const now = Date.now();
  if (parsed < now - 46 * 86400000 || parsed > now + 2 * 86400000) return day ? `${day}T12:00:00.000Z` : new Date().toISOString();
  return new Date(parsed).toISOString();
}
function rc54SanitizeEvent(raw = {}) {
  const eventType = safeText(raw.type || raw.eventType || '', 40);
  if (!RC54_ANALYTICS_EVENTS.has(eventType)) return null;
  const playerId = safeText(raw.playerId || '', 90);
  const eventId = safeText(raw.eventId || '', 220);
  const day = rc54SafeDay(raw.day || raw.eventDay || '');
  const firstSeenDay = rc54SafeDay(raw.firstSeenDay || day, 1000) || day;
  if (!playerId || !eventId || !day || !firstSeenDay) return null;
  const source = safeText(raw.source || raw.meta?.source || '', 48);
  const disciplineId = safeText(raw.disciplineId || raw.meta?.disciplineId || '', 32);
  const mysteryId = safeText(raw.mysteryId || raw.meta?.mysteryId || '', 90);
  const lessonId = safeText(raw.lessonId || raw.meta?.lessonId || '', 90);
  const metadata = {};
  const boundedNumber = (value, min, max) => Number.isFinite(Number(value)) ? Math.max(min, Math.min(max, Number(value))) : null;
  const gapDays = boundedNumber(raw.meta?.gapDays, 0, 90); if (gapDays !== null) metadata.gapDays = gapDays;
  const tries = boundedNumber(raw.meta?.tries, 0, 50); if (tries !== null) metadata.tries = tries;
  const hints = boundedNumber(raw.meta?.hints, 0, 10); if (hints !== null) metadata.hints = hints;
  return {
    event_id: eventId,
    player_id: playerId,
    event_type: eventType,
    event_day: day,
    occurred_at: rc54SafeIso(raw.at || raw.occurredAt || '', day),
    first_seen_day: firstSeenDay,
...
async function rc54AnalyticsSchemaProbe() {
  if (!configured()) return { configured: false, ready: false, mode: 'not-configured' };
  try {
    await request('hd_analytics_events?select=event_id&limit=1');
    return { configured: true, ready: true, mode: 'supabase' };
  } catch (error) {
    return { configured: true, ready: false, mode: 'schema-missing', message: 'Table hd_analytics_events absente ou inaccessible.' };
  }
}
async function rc54HandleAnalyticsEvents(req, res) {
  if (String(req.method || 'GET').toUpperCase() !== 'POST') return send(res, 405, { ok: false, message: 'POST requis.', version: VERSION });
  const body = readBody(req);
  const incoming = Array.isArray(body.events) ? body.events.slice(0, 24) : [];
  const rows = incoming.map(rc54SanitizeEvent).filter(Boolean);
  if (!rows.length) return send(res, 400, { ok: false, stored: false, accepted: 0, message: 'Aucun événement valide.', version: VERSION });
  if (!configured()) return send(res, 200, { ok: true, stored: false, accepted: 0, mode: 'local-only', message: 'Analytics en attente : Supabase non configuré.', version: VERSION });
  try {
    await request('hd_analytics_events?on_conflict=event_id', {
      method: 'POST',
      body: rows,
    ...
    return send(res, 200, { ok: true, stored: true, accepted: rows.length, mode: 'supabase', version: VERSION });
  } catch (error) {
    const missing = Number(error?.status || 0) === 404 || String(error?.body?.message || '').includes('hd_analytics_events');
    return send(res, 200, { ok: true, stored: false, accepted: 0, mode: missing ? 'analytics-table-missing' : 'supabase-error', message: missing ? 'Migration RC54 analytics à appliquer.' : 'Analytics conservés localement pour une prochaine synchro.', version: VERSION });
  }
}
function rc54Authorized(req) {
  const expected = String(process.env.ANALYTICS_ADMIN_TOKEN || '');
  if (!expected || expected.length < 24) return false;
  const auth = String(req.headers?.authorization || '');
  const headerToken = String(req.headers?.['x-analytics-admin-token'] || '');
  const supplied = auth.startsWith('Bearer ') ? auth.slice(7).trim() : headerToken.trim();
  if (!supplied || supplied.length !== expected.length) return false;
  let mismatch = 0;
  for (let i = 0; i < expected.length; i += 1) mismatch |= expected.charCodeAt(i) ^ supplied.charCodeAt(i);
  return mismatch === 0;
}
function rc54Rate(numerator, denominator) {
  return denominator > 0 ? Math.round((numerator / denominator) * 1000) / 10 : null;
}
function rc54DayDiff(a, b) {
  const left = dayIndexFromKey(a); const right = dayIndexFromKey(b);
  return left === null || right === null ? null : right - left;
}
function rc54AggregateAnalytics(rows = [], nowDay = todayKey()) {
  const players = new Map();
  for (const raw of rows || []) {
    const playerId = safeText(raw.player_id || '', 90); if (!playerId) continue;
    const day = safeText(raw.event_day || '', 10); const firstSeen = safeText(raw.first_seen_day || day, 10); const type = safeText(raw.event_type || '', 40);
    if (!day || !firstSeen || !type) continue;
    const item = players.get(playerId) || { playerId, firstSeen, events: [], activeDays: new Set(), firstDiscipline: '' };
    if (firstSeen < item.firstSeen) item.firstSeen = firstSeen;
    item.events.push({ type, day, disciplineId: safeText(raw.discipline_id || '', 32), at: raw.occurred_at || '' });
    if (type === 'app_open') item.activeDays.add(day);
    players.set(playerId, item);
  }
  for (const item of players.values()) {
    item.events.sort((a, b) => String(a.at).localeCompare(String(b.at)));
    item.firstDiscipline = item.events.find(event => event.type === 'mystery_start' && event.disciplineId)?.disciplineId || '';
  }
  const cohorts = new Map();
  const disciplineGroups = new Map();
  const todayIndex = dayIndexFromKey(nowDay);
  const totals = { users: 0, opened: 0, onboarding: 0, started: 0, solved: 0, deepDive: 0, eligibleD1: 0, D1: 0, eligibleD3: 0, D3: 0, eligibleD7: 0, D7: 0 };
  for (const item of players.values()) {
    totals.users += 1;
    const firstEvents = item.events.filter(event => event.day === item.firstSeen);
    const types = new Set(firstEvents.map(event => event.type));
    if (types.has('app_open')) totals.opened += 1;
    if (types.has('onboarding_completed')) totals.onboarding += 1;
    if (types.has('mystery_start')) totals.started += 1;
    if (types.has('mystery_solved')) totals.solved += 1;
    if (types.has('deep_dive')) totals.deepDive += 1;
    const age = todayIndex === null ? null : todayIndex - dayIndexFromKey(item.firstSeen);
    const retained = offset => item.activeDays.has((() => { const d = new Date(`${item.firstSeen}T12:00:00Z`); d.setUTCDate(d.getUTCDate() + offset); return d.toISOString().slice(0,10); })());
    if (age !== null && age >= 1) { totals.eligibleD1 += 1; if (retained(1)) totals.D1 += 1; }
    if (age !== null && age >= 3) { totals.eligibleD3 += 1; if (retained(3)) totals.D3 += 1; }
    if (age !== null && age >= 7) { totals.eligibleD7 += 1; if (retained(7)) totals.D7 += 1; }
    const cohort = cohorts.get(item.firstSeen) || { day: item.firstSeen, users: 0, started: 0, solved: 0, deepDive: 0, eligibleD1: 0, D1: 0, eligibleD3: 0, D3: 0, eligibleD7: 0, D7: 0 };
    cohort.users += 1; if (types.has('mystery_start')) cohort.started += 1; if (types.has('mystery_solved')) cohort.solved += 1; if (types.has('deep_dive')) cohort.deepDive += 1;
    if (age !== null && age >= 1) { cohort.eligibleD1 += 1; if (retained(1)) cohort.D1 += 1; }
    if (age !== null && age >= 3) { cohort.eligibleD3 += 1; if (retained(3)) cohort.D3 += 1; }
    if (age !== null && age >= 7) { cohort.eligibleD7 += 1; if (retained(7)) cohort.D7 += 1; }
    cohorts.set(item.firstSeen, cohort);
    const discipline = item.firstDiscipline || 'unknown';
    const group = disciplineGroups.get(discipline) || { discipline, users: 0, solved: 0, eligibleD1: 0, D1: 0, eligibleD7: 0, D7: 0 };
    group.users += 1; if (types.has('mystery_solved')) group.solved += 1;
    if (age !== null && age >= 1) { group.eligibleD1 += 1; if (retained(1)) group.D1 += 1; }
    if (age !== null && age >= 7) { group.eligibleD7 += 1; if (retained(7)) group.D7 += 1; }
    disciplineGroups.set(discipline, group);
  }
  const decorate = row => ({ ...row, startRate: rc54Rate(row.started || 0, row.users), solveRate: rc54Rate(row.solved || 0, row.users), deepDiveRate: rc54Rate(row.deepDive || 0, row.users), D1Rate: rc54Rate(row.D1 || 0, row.eligibleD1 || 0), D3Rate: rc54Rate(row.D3 || 0, row.eligibleD3 || 0), D7Rate: rc54Rate(row.D7 || 0, row.eligibleD7 || 0) });
  return {
    users: totals.users,
    firstSession: { opened: totals.opened, onboarding: totals.onboarding, started: totals.started, solved: totals.solved, deepDive: totals.deepDive, startRate: rc54Rate(totals.started, totals.users), solveRate: rc54Rate(totals.solved, totals.users), deepDiveRate: rc54Rate(totals.deepDive, totals.users) },
    retention: { D1: totals.D1, eligibleD1: totals.eligibleD1, D1Rate: rc54Rate(totals.D1, totals.eligibleD1), D3: totals.D3, eligibleD3: totals.eligibleD3, D3Rate: rc54Rate(totals.D3, totals.eligibleD3), D7: totals.D7, eligibleD7: totals.eligibleD7, D7Rate: rc54Rate(totals.D7, totals.eligibleD7) },
    cohorts: Array.from(cohorts.values()).sort((a,b)=>b.day.localeCompare(a.day)).map(decorate),
    byDiscipline: Array.from(disciplineGroups.values()).sort((a,b)=>b.users-a.users).map(row=>({ ...row, solveRate: rc54Rate(row.solved,row.users), D1Rate: rc54Rate(row.D1,row.eligibleD1), D7Rate: rc54Rate(row.D7,row.eligibleD7) }))
  };
}
async function rc54AnalyticsRows(days = 30) {
  const today = dayIndexFromKey(todayKey());
  const start = new Date(); start.setHours(12,0,0,0); start.setDate(start.getDate() - Math.max(7, Math.min(120, days)) - 7);
  const startKey = todayKey(start);
  const select = 'event_id,player_id,event_type,event_day,occurred_at,first_seen_day,discipline_id';
  const out = []; const pageSize = 1000; const maxRows = 12000;
  for (let offset=0; offset<maxRows; offset+=pageSize) {
    const rows = await request(`hd_analytics_events?select=${select}&first_seen_day=gte.${encodeURIComponent(startKey)}&order=occurred_at.asc&limit=${pageSize}&offset=${offset}`);
    if (!Array.isArray(rows) || !rows.length) break;
    out.push(...rows); if (rows.length < pageSize) break;
  }
  return out;
}
async function rc54HandleAnalyticsCohorts(req, res) {
  if (!rc54Authorized(req)) return send(res, 404, { ok: false, message: 'Route API inconnue', version: VERSION });
  if (!configured()) return send(res, 503, { ok: false, mode: 'not-configured', message: 'Supabase non configuré.', version: VERSION });
  const days = Math.max(7, Math.min(120, Number(req.query?.days || 30)));
  try {
    const rows = await rc54AnalyticsRows(days);
    const summary = rc54AggregateAnalytics(rows, todayKey());
    const cutoff = (() => { const d=new Date(); d.setHours(12,0,0,0); d.setDate(d.getDate()-days+1); return todayKey(d); })();
    summary.cohorts = summary.cohorts.filter(row => row.day >= cutoff);
    return send(res, 200, { ok: true, mode: 'supabase', version: VERSION, days, generatedAt: new Date().toISOString(), ...summary });
  } catch (error) {
    return send(res, 503, { ok: false, mode: 'analytics-unavailable', message: 'Analytics indisponibles : vérifier la migration RC54.', version: VERSION });
  }
}
const rc54BaseHandleRoute = handleRoute;
handleRoute = async function rc54HandleRoute(req, res, route) {
  if (route === 'analytics/events') return rc54HandleAnalyticsEvents(req, res);
  if (route === 'analytics/cohorts') return rc54HandleAnalyticsCohorts(req, res);
  return rc54BaseHandleRoute(req, res, route);
};
const rc54BaseHealth = handleHealth;
handleHealth = async function rc54HandleHealth(req, res) {
  const fake = { status: code => ({ json: payload => ({ code, payload }) }), setHeader: () => {} };
  try {
    const base = await rc54BaseHealth(req, fake); const payload = base?.payload || {};
    const analytics = await rc54AnalyticsSchemaProbe();
    return send(res, 200, { ...payload, version: VERSION, deployment: { ...(payload.deployment || {}), analyticsReady: Boolean(analytics.ready), analyticsMode: analytics.mode }, features: Array.from(new Set([...(payload.features || []), 'central-retention-analytics', 'cohort-d1-d3-d7'])) });
  } catch { return rc54BaseHealth(req, res); }
};

module.exports = { handleRoute, handleRequest, VERSION, _rc54AggregateAnalytics: rc54AggregateAnalytics, _rc54SanitizeEvent: rc54SanitizeEvent };
