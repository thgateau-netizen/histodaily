...
/* =========================================================
   Beta 161 — réparation relation ami côté API
   - quand une relation existe déjà dans hd_friends, les demandes pending
     obsolètes sont neutralisées côté réponse
   - /friends/requests renvoie aussi friends pour aider le front à se réparer
   ========================================================= */
const BETA161_API_VERSION = '1.0.0-beta.161';
function beta161SameCode(a = '', b = '') { return friendCodeMatches(cleanFriendCode(a), cleanFriendCode(b)); }
function beta161SameFriend(row = {}, req = {}) {
  const fid = safeText(row.friend_player_id || row.playerId || row.player_id || '', 90);
  const fcode = cleanFriendCode(row.friend_code || row.friendCode || row.code || '');
  const rid = safeText(req.otherPlayerId || req.requesterPlayerId || req.targetPlayerId || '', 90);
  const rcode = cleanFriendCode(req.otherFriendCode || req.requesterFriendCode || req.targetFriendCode || '');
  return Boolean((fid && rid && fid === rid) || (fcode && rcode && beta161SameCode(fcode, rcode)));
}
function beta161SplitRequestsForClient(rows = [], friends = []) {
  const pending = (rows || []).filter(row => row.status === 'pending' && !(friends || []).some(friend => beta161SameFriend(friend, row)));
  return {
    incoming: pending.filter(row => row.direction === 'incoming'),
    outgoing: pending.filter(row => row.direction === 'outgoing'),
    history: (rows || []).filter(row => row.status !== 'pending' || (friends || []).some(friend => beta161SameFriend(friend, row))).slice(0, 20)
  };
}
async function beta161HealAcceptedRows(playerId = '', myFriendCode = '') {
  const rows = await friendRequestRowsForPlayer(playerId, myFriendCode).catch(() => []);
  const accepted = (rows || []).filter(row => row.status === 'accepted');
  for (const row of accepted.slice(0, 10)) {
    const meProfile = await profileByPlayerId(playerId).catch(() => null);
    const otherProfile = row.otherPlayerId ? await profileByPlayerId(row.otherPlayerId).catch(() => null) : await profileByFriendCode(row.otherFriendCode).catch(() => null);
    const me = {
      playerId: safeText(meProfile?.player_id || playerId || '', 90),
      friendCode: cleanFriendCode(meProfile?.friend_code || myFriendCode || ''),
      pseudo: safeText(meProfile?.pseudo || 'Joueur', 32)
    };
    const other = {
      playerId: safeText(otherProfile?.player_id || row.otherPlayerId || '', 90),
      friendCode: cleanFriendCode(otherProfile?.friend_code || row.otherFriendCode || ''),
      pseudo: safeText(otherProfile?.pseudo || row.otherPseudo || 'Joueur', 32)
    };
    if (me.playerId && other.friendCode) await upsertFriendPair(me, other).catch(() => null);
  }
}
const beta161PreviousHandleFriendRequests = handleFriendRequests;
handleFriendRequests = async function beta161HandleFriendRequests(req, res) {
  const body = readBody(req);
  const method = req.method || 'GET';
  let playerId = safeText(body.playerId || req.query?.playerId || '', 90);
  let myFriendCode = cleanFriendCode(body.myFriendCode || body.friendCodeSelf || body.selfFriendCode || req.query?.friendCode || req.query?.myFriendCode || '');
  if (!configured()) return beta161PreviousHandleFriendRequests(req, res);
  try {
    if (method === 'GET') {
      await beta161HealAcceptedRows(playerId, myFriendCode).catch(() => null);
      const rows = await friendRequestRowsForPlayer(playerId, myFriendCode).catch(() => []);
      const friends = await currentFriendRows(playerId).catch(() => []);
      return send(res, 200, { ok: true, mode: 'supabase', stored: true, requests: beta161SplitRequestsForClient(rows, friends), friends, features: { ...requestFeatureFlags(), relationRepair: true }, message: friends.length ? 'Relation amis vérifiée.' : '' });
    }
  } catch {}
  return beta161PreviousHandleFriendRequests(req, res);
};
const beta161PreviousRequestFeatureFlags = requestFeatureFlags;
requestFeatureFlags = function beta161RequestFeatureFlags() {
  return { ...beta161PreviousRequestFeatureFlags(), relationRepair: true, pendingCleanup: true };
};
