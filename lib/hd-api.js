...
function aggregate(rows, scope) {
  ...
  for (const row of rows || []) {
    ...
    const totalXpScore = Math.max(Number(row.xp || 0), Number(row.score || 0));
    existing.score = Math.max(existing.score, totalXpScore);
...
async function handleSubmit(req, res) {
  ...
  const score = Math.max(0, Math.min(999999, Number(body.totalXp || body.xp || body.score || 0)));
