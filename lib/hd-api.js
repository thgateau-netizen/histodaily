...
async function handleSubmit(req, res) {
  ...
  if (!expectedMystery || expectedMystery.id !== mysteryId) return send(res, 200, { ok: true, stored: false, skipped: true, terminal: true, mode: 'not-ranked', message: 'Ce dossier ne correspond pas au mystère quotidien de cette date.' });
...
/* HistoDaily RC13.1 — Web Push routes. */
const { handlePushRoute } = require('./hd-push');
const rc131BaseHandleRoute = handleRoute;
handleRoute = async function rc131PushHandleRoute(req, res, route) {
  if (String(route || '').startsWith('push/')) {
    const handled = await handlePushRoute(req, res, route, send);
    if (handled !== null && handled !== undefined) return handled;
  }
  return rc131BaseHandleRoute(req, res, route);
};
