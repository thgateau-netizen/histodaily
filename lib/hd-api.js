...
handleSubmit = async function beta249HandleSubmit(req, res) {
  ...
  const expectedMystery = dailyMysteryForKey(periodKey);
  if (!expectedMystery || expectedMystery.id !== mysteryId) {
    return send(res, 200, {
      ok: true, stored: false, skipped: true, terminal: true, mode: 'not-ranked',
      message: 'Ce dossier ne correspond pas au mystère quotidien de cette date.'
    });
  }
