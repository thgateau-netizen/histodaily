...
/* =========================================================
   Beta 243 — vérité serveur pour scores et relations sociales
   ...
   ========================================================= */
async function beta243ResolveIdentity(playerId = '', friendCode = '', pseudo = 'Joueur') {
...
const beta243PreviousHandleFriends = handleFriends;
handleFriends = async function beta243HandleFriends(req, res) {
  const method = req.method || 'GET';
  if (method !== 'GET') return beta243PreviousHandleFriends(req, res);
  if (!configured()) return beta243PreviousHandleFriends(req, res);
  try {
    ...
    const identity = await beta243ResolveIdentity(requestedId, requestedCode);
...
handleLeaderboard = async function beta243HandleLeaderboard(req, res) {
  const rawScope = Array.isArray(req.query?.scope) ? req.query.scope[0] : req.query?.scope;
  const scope = ['daily', 'week', 'year', 'friends'].includes(rawScope) ? rawScope : 'daily';
  const rawPeriod = Array.isArray(req.query?.periodKey) ? req.query.periodKey[0] : req.query?.periodKey;
  const periodKey = safeText(rawPeriod || todayKey(), 20);
  ...
  try {
    ...
    if (scope === 'friends') {
      const identity = await beta243ResolveIdentity(
      ...
      const ids = new Set([canonicalPlayerId].filter(Boolean));
      const codes = new Set([canonicalFriendCode].filter(Boolean));
      for (const friend of friends) {
        const id = safeText(friend.friend_player_id || friend.playerId || '', 90);
        const code = cleanFriendCode(friend.friend_code || friend.friendCode || friend.code || '');
        if (id) ids.add(id);
...
handleSubmit = async function beta243HandleSubmit(req, res) {
  if (req.method && req.method !== 'POST') { res.setHeader('Allow', 'POST'); return send(res, 405, { ok: false, message: 'POST only' }); }
  const body = readBody(req);
...
handleFriendRequestRespond = async function beta243HandleFriendRequestRespond(req, res) {
  if (req.method && req.method !== 'POST') { res.setHeader('Allow', 'POST'); return send(res, 405, { ok: false, message: 'POST only' }); }
  const body = readBody(req);
  const responseInput = safeText(body.response || body.status || '', 20);
  ...
  try {
    const me = await beta243ResolveIdentity(body.playerId || '', body.myFriendCode || body.friendCodeSelf || '', body.pseudo || 'Joueur');
    const requester = await beta243ResolveIdentity(body.requesterPlayerId || body.otherPlayerId || '', body.requesterFriendCode || body.otherFriendCode || '', body.requesterPseudo || 'Joueur');
...
const beta243PreviousHandleHealth = handleHealth;
handleHealth = async function beta243HandleHealth(req, res) {
  const fake = { status: (code) => ({ json: (payload) => ({ code, payload }) }), setHeader: () => {} };
  try {
    const base = await beta243PreviousHandleHealth(req, fake);
    const payload = base?.payload || {};
    return send(res, 200, {
      ...payload,
      version: VERSION,
    ...
    return beta243PreviousHandleHealth(req, res);
  }
};
