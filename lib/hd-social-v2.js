...
async function friendRows(playerId = '') {
  const id = cleanId(playerId);
  if (!id) return [];
  const rows = await request(`hd_friends?select=player_id,friend_player_id,friend_code,friend_pseudo,created_at&player_id=eq.${encodeURIComponent(id)}&order=created_at.asc`);
  const ids = [...new Set((rows || []).map(row => cleanId(row.friend_player_id)).filter(Boolean))];
  const codes = [...new Set((rows || []).map(row => cleanCode(row.friend_code)).filter(Boolean))];
  ...
  for (const row of rows || []) {
    const storedId = cleanId(row.friend_player_id);
    const storedCode = cleanCode(row.friend_code);
    const profile = byId.get(storedId) || byCode.get(storedCode) || null;
    const friendId = cleanId(profile?.player_id || storedId);
    const code = cleanCode(profile?.friend_code || storedCode);
    const key = friendId || code;
    ...
    friends.push({
      ...
      friendCode: code,
      code,
      pseudo: cleanPseudo(profile?.pseudo || row.friend_pseudo || 'Ami'),
      name: cleanPseudo(profile?.pseudo || row.friend_pseudo || 'Ami'),
...
async function snapshot(identity) {
  const [friends, requests] = await Promise.all([
    friendRows(identity.canonicalPlayerId),
    requestRows(identity.canonicalPlayerId, identity.canonicalFriendCode)
  ]);
  return { friends, requests };
}

function aggregateScores(rows = [], profiles = [], includeIds = []) {
  ...
  for (const row of rows || []) {
    ...
    const key = `${safeText(row.period_key || '', 20)}|${safeText(row.mystery_id || '', 100) || safeText(row.solved_at || '', 40)}`;
    if (!scoreKeys.has(playerId)) scoreKeys.set(playerId, new Set());
    if (scoreKeys.get(playerId).has(key)) continue;
    scoreKeys.get(playerId).add(key);
    const profile = profileMap.get(playerId) || {};
    const current = players.get(playerId) || {
      playerId,
      id: playerId,
      pseudo: cleanPseudo(profile.pseudo || row.pseudo || 'Joueur'),
      name: cleanPseudo(profile.pseudo || row.pseudo || 'Joueur'),
      friendCode: cleanCode(profile.friend_code || row.friend_code || ''),
      code: cleanCode(profile.friend_code || row.friend_code || ''),
      score: 0,
      solvedInPeriod: 0,
      level: Math.max(int(profile.level, 1), int(row.level, 1)),
      xp: Math.max(int(profile.xp), int(row.xp)),
      solvedCount: Math.max(int(profile.solved_count), int(row.solved_count)),
      streak: Math.max(int(profile.streak), int(row.streak))
    };
    current.score += Math.min(int(row.score), scoreCap(row.difficulty));
  ...
  for (const rawId of includeIds || []) {
    ...
    players.set(playerId, {
      playerId,
      id: playerId,
      pseudo: cleanPseudo(profile.pseudo || 'Joueur'),
      name: cleanPseudo(profile.pseudo || 'Joueur'),
      friendCode: cleanCode(profile.friend_code || ''),
      code: cleanCode(profile.friend_code || ''),
      score: 0,
      solvedInPeriod: 0,
      level: int(profile.level, 1),
      xp: int(profile.xp),
  ...
  const sorted = [...players.values()].sort((a, b) => b.score - a.score || a.pseudo.localeCompare(b.pseudo, 'fr'));
...
async function leaderboardRows({ period, audience, identity, query }) {
  ...
  if (audience === 'friends') {
    const friends = await friendRows(identity.canonicalPlayerId);
  ...
  if (audience === 'friends') {
    if (!includeIds.length) return { rows: [], bounds };
  ...
  let rows = aggregateScores(scores || [], profiles || [], includeIds);
  if (audience === 'general') rows = rows.filter(row => row.score > 0 || row.playerId === identity.canonicalPlayerId);
  return { rows: rows.slice(0, audience === 'friends' ? 250 : 100), bounds };
}
...
async function handleHealth(req, res) {
  ...
  return send(res, ready ? 200 : 503, {
    ...
    features: ['profiles', 'atomic-profile-merge', 'friend-requests', 'terminal-request-decisions', 'reciprocal-friends', 'daily-week-year-leaderboards', 'zero-score-friends', 'atomic-best-score']
  });
}
...
module.exports = {
  VERSION,
  handleSocialV2,
  _test: { cleanCode, cleanId, periodBounds, aggregateScores, scoreCap, profilePayload, upsertBestScore }
};
