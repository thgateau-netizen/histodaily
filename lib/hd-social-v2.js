...
const { dailyMysteryForKey, dayIndexFromKey } = require('./hd-data');
...
async function handleScore(req, res) {
  ...
  if (!expectedMystery || expectedMystery.id !== mysteryId) {
    return send(res, 200, { ok: true, stored: false, skipped: true, terminal: true, mode: 'not-ranked', authoritative: true, message: 'Ce dossier ne correspond pas au mystère quotidien de cette date.' });
  }
