...
const { mysteryById, dayIndexFromKey } = require('./hd-data');
...
function scoreFloor(difficulty = '') {
  switch (String(difficulty || '').toLowerCase()) {
    case 'facile': return 35;
    case 'difficile': return 55;
    case 'expert': return 70;
    default: return 45;
  }
}

function canonicalMysteryScore(difficulty = 'moyen', hints = 0, tries = 1) {
  const base = scoreCap(difficulty);
  const floor = scoreFloor(difficulty);
  const safeHints = int(hints, 0, 3);
  const safeTries = int(tries, 1, 50);
  return Math.max(floor, base - safeHints * 20 - Math.max(0, safeTries - 1) * 10);
}

function validRecentPeriodKey(value = '', maxAgeDays = 31) {
...
function canonicalSolvedAt(periodKey = '') {
  return `${periodKey}T12:00:00.000Z`;
}
...
function limitRowsPreservingIds(rows = [], limit = 100, mustKeepIds = []) {
  const safeLimit = Math.max(1, int(limit, 1, 1000));
  const keep = new Set((mustKeepIds || []).map(cleanId).filter(Boolean));
  const head = rows.slice(0, safeLimit);
  const seen = new Set(head.map(row => cleanId(row.playerId || row.id)).filter(Boolean));
  const forced = rows.filter(row => {
    const id = cleanId(row.playerId || row.id);
    return id && keep.has(id) && !seen.has(id);
  });
  return [...head, ...forced];
}
...
function aggregateScores(rows = [], profiles = [], includeIds = [], seedPlayers = []) {
  ...
  // Une journée ne doit compter qu'une seule expédition par joueur. Les anciennes
  // versions pouvaient laisser plusieurs IDs pour la même date ; on conserve ici
  // le meilleur score de la journée avant d'agréger la semaine.
  const bestDailyRows = new Map();
  for (const rawScoreRow of rows || []) {
    const playerId = cleanId(rawScoreRow.player_id);
    if (!playerId) continue;
    const dayKey = safeText(rawScoreRow.period_key || String(rawScoreRow.solved_at || '').slice(0, 10), 20);
    const dailyKey = `${playerId}|${dayKey || safeText(rawScoreRow.mystery_id || rawScoreRow.solved_at || '', 100)}`;
    const current = bestDailyRows.get(dailyKey);
    const candidateScore = Math.min(int(rawScoreRow.score), scoreCap(rawScoreRow.difficulty));
    const currentScore = current ? Math.min(int(current.score), scoreCap(current.difficulty)) : -1;
    if (!current || candidateScore > currentScore) bestDailyRows.set(dailyKey, rawScoreRow);
  }

  for (const scoreRow of bestDailyRows.values()) {
...
function rankRowsByScore(rows = []) {
  const sorted = [...(rows || [])].sort((a, b) => int(b.score) - int(a.score) || cleanPseudo(a.pseudo || a.name || 'Joueur').localeCompare(cleanPseudo(b.pseudo || b.name || 'Joueur'), 'fr') || cleanCode(a.friendCode || a.code || '').localeCompare(cleanCode(b.friendCode || b.code || '')));
  let previousScore = null;
  let previousRank = 0;
  return sorted.map((row, index) => {
    const score = int(row.score);
    const rank = previousScore !== null && previousScore === score ? previousRank : index + 1;
    previousScore = score;
    previousRank = rank;
...
function annualXpRows(rows = []) {
  // RC30: l'application ayant été lancée cette année, le profil XP est la
  // meilleure source historique disponible pour reprendre les comptes existants.
  // Contrairement à hd_scores, il inclut cours, quiz, révisions et ateliers.
  return rankRowsByScore((rows || []).map(row => ({
    ...row,
    score: int(row.xp),
    scoreMetric: 'xp',
    solvedInPeriod: int(row.solvedCount)
  })));
}

async function allRegisteredProfiles(maxRows = 2500) {
  const pageSize = 500;
  const out = [];
  for (let offset = 0; offset < maxRows; offset += pageSize) {
    const page = await request(`hd_profiles?select=player_id,pseudo,friend_code,level,xp,solved_count,streak,updated_at&order=updated_at.desc.nullslast&limit=${pageSize}&offset=${offset}`).catch(() => []);
...
async function leaderboardRows({ period, audience, identity, query }) {
  ...
  if (audience === 'general') {
    registeredProfiles = await allRegisteredProfiles();
    seedPlayers.push(...registeredProfiles.map(identityShape));
  }
  if (audience === 'friends') {
  ...
  // L'annuel repose sur l'XP du profil : c'est la seule donnée historique qui
  // couvre déjà toutes les formes d'apprentissage pour les comptes existants.
  // Jour/Semaine restent basés sur les points d'expédition horodatés.
  const scores = bounds.period === 'year'
    ? []
    : await request(`hd_scores?select=player_id,pseudo,friend_code,mystery_id,period_key,score,difficulty,level,xp,solved_count,streak,solved_at&${filter}&order=score.desc&limit=${limit}`);
  const profileIds = [...new Set([...(scores || []).map(row => cleanId(row.player_id)), ...includeIds].filter(Boolean))];
  const scoreProfiles = await profilesByIds(profileIds).catch(() => []);
  const profileMap = new Map();
  [...registeredProfiles, ...(scoreProfiles || [])].forEach(profile => {
    const id = cleanId(profile.player_id);
    if (id) profileMap.set(id, profile);
  });
  const profiles = [...profileMap.values()];
  const aggregated = aggregateScores(scores || [], profiles || [], includeIds, seedPlayers);
  const rows = bounds.period === 'year' ? annualXpRows(aggregated) : aggregated;
  const zeroScoreFriendCount = audience === 'friends'
    ...
    : 0;
  const visibleRows = audience === 'general'
    ? rows
    : limitRowsPreservingIds(rows, 250, [identity.canonicalPlayerId]);
  return {
    rows: visibleRows,
    bounds,
    friendCount: friends.length,
    zeroScoreFriendCount,
    registeredPlayerCount: audience === 'general' ? visibleRows.length : undefined,
    zeroScorePlayerCount: audience === 'general' ? visibleRows.filter(row => int(row.score) === 0).length : undefined
  };
}
...
async function handleLeaderboard(req, res) {
  ...
  return send(res, 200, {
    ...
    period,
    audience,
    scoreMetric: period === 'year' ? 'xp' : 'mystery-points',
    ...
    registeredPlayerCount: result.registeredPlayerCount || rows.length,
    zeroScorePlayerCount: result.zeroScorePlayerCount || 0,
    generatedAt: new Date().toISOString()
  });
}
...
async function handleScore(req, res) {
  if (String(req.method || 'GET').toUpperCase() !== 'POST') return send(res, 405, { ok: false, message: 'POST requis.' });
  const body = readBody(req);
  const explicitlyIneligible = body.rankingEligible === false || body.daily === false || body.archive === true;
  ...
  const periodKey = validRecentPeriodKey(body.dayKey || body.periodKey || todayKey());
  if (!periodKey) return send(res, 400, { ok: false, stored: false, terminal: true, message: 'Période de score invalide ou trop ancienne.' });
  // RC46 : le dossier quotidien est personnalisé côté client (discipline, historique,
  // difficulté adaptative). Le serveur ne peut donc plus imposer un ID global par date.
  // Il valide à la place que le dossier appartient au catalogue canonique partagé.
  const expectedMystery = mysteryById(mysteryId);
  if (!expectedMystery) {
    return send(res, 200, { ok: true, stored: false, skipped: true, terminal: true, mode: 'not-ranked', authoritative: true, message: 'Dossier quotidien inconnu du catalogue.' });
  }
  if (!configured()) return send(res, 503, { ok: false, stored: false, mode: 'not-configured', terminal: false, message: 'Score gardé localement : Supabase non configuré.' });
  const identity = await resolveIdentity(body);

  // Une seule expédition peut compter au classement par jour. Cela évite qu'un
  // changement de rotation, une restauration ou un client modifié cumule plusieurs
  // dossiers sur la même date. Une nouvelle tentative du même dossier reste permise
  // afin de conserver la logique « meilleur score ».
  const existingDaily = await request(`hd_scores?select=mystery_id,score,difficulty,hints,tries&player_id=eq.${encodeURIComponent(identity.canonicalPlayerId)}&period_key=eq.${encodeURIComponent(periodKey)}&scope=eq.${SCORE_SCOPE}&order=score.desc&limit=1`).catch(() => []);
  if (existingDaily?.[0] && String(existingDaily[0].mystery_id || '') !== mysteryId) {
    return send(res, 200, {
      ok: true, stored: false, skipped: true, terminal: true, mode: 'daily-already-ranked', authoritative: true,
      exactScore: int(existingDaily[0].score), existingMysteryId: safeText(existingDaily[0].mystery_id || '', 100),
      canonicalPlayerId: identity.canonicalPlayerId, canonicalFriendCode: identity.canonicalFriendCode,
      message: 'Une expédition est déjà comptée pour cette journée.'
    });
  }

  const difficulty = safeText(expectedMystery.difficulty || 'moyen', 20);
  const hints = int(body.hints, 0, 3);
  const tries = int(body.tries || body.answerTries, 1, 50);
  const payload = {
    ...
    scope: SCORE_SCOPE,
    score: canonicalMysteryScore(difficulty, hints, tries),
    hints,
    tries,
    difficulty,
    ...
    solved_at: canonicalSolvedAt(periodKey)
  };
...
async function handleFriendRequest(req, res) {
  ...
  const body = readBody(req);
  const targetPlayerId = cleanId(body.targetPlayerId || body.friendPlayerId || body.playerIdTarget || '');
  const targetCode = cleanCode(body.targetFriendCode || body.friendCodeTarget || body.code || '');
  if (!targetPlayerId && !targetCode) return send(res, 400, { ok: false, message: 'Joueur cible requis.' });
  const meIdentity = await resolveIdentity(body);
  const me = identityShape(meIdentity.profile);
  let targetProfile = targetPlayerId ? await profileById(targetPlayerId).catch(() => null) : null;
  if (!targetProfile && targetCode) targetProfile = await profileByCode(targetCode).catch(() => null);
  if (!targetProfile && targetPlayerId) targetProfile = await profileByCode(targetPlayerId).catch(() => null);
  if (!targetProfile) return send(res, 404, { ok: false, message: 'Ce profil joueur est introuvable.' });
  const target = identityShape(targetProfile);
  if (target.playerId === me.playerId || (target.friendCode && me.friendCode && target.friendCode === me.friendCode)) return send(res, 400, { ok: false, message: 'Tu ne peux pas t’ajouter toi-même.' });
...
async function handleProfile(req, res) {
  ...
  const query = queryOf(req);
  let profile = query.playerId ? await profileById(query.playerId).catch(() => null) : null;
  if (!profile) profile = await profileByCode(query.friendCode || query.code || query.playerId || '').catch(() => null);
  ...
  const results = await Promise.all(periods.map(period => leaderboardRows({ period, audience: 'general', identity, query })));
  periods.forEach((period, index) => {
    const row = results[index].rows.find(item => item.playerId === target.playerId) || null;
    scores[period] = row?.score || 0;
    ranks[period] = row?.rank || 0;
  });
  const publicProfile = {
...
async function handleHealth(req, res) {
  ...
  return send(res, ready ? 200 : 503, {
    ...
    features: ['profiles', 'atomic-profile-merge', 'friend-requests', 'terminal-request-decisions', 'reciprocal-friends', 'daily-week-year-leaderboards', 'year-xp-progression-ranking', 'zero-score-friends', 'one-sided-friend-read-recovery', 'friend-code-zero-score-seeding', 'accepted-request-friend-recovery', 'reciprocal-friend-auto-heal', 'continuous-score-outbox-recovery', 'atomic-best-score', 'ranking-profile-friend-requests', 'player-id-friend-targeting', 'preserve-current-player-outside-top-100', 'parallel-public-profile-ranks', 'archive-score-exclusion']
  });
}
...
module.exports = {
  VERSION,
  handleSocialV2,
  _test: { cleanCode, cleanId, periodBounds, aggregateScores, annualXpRows, rankRowsByScore, limitRowsPreservingIds, friendRows, leaderboardRows, scoreCap, scoreFloor, canonicalMysteryScore, validRecentPeriodKey, canonicalSolvedAt, profilePayload, upsertBestScore }
};
